...
The native pump task owns the display on both platforms, so the driver is one build + poll
loop with no per-platform branch. These tests fake the native module and drive that loop
deterministically.
"""
import sys
import time
