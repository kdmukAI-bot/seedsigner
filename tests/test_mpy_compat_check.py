...
DETECT_CASES = [
    ...
    ("format_builtin",         21, "s = format(x, '011b')\n"),
    ("random_shuffle",         21, "random.shuffle(items)\n"),
...
CLEAN_CASES = [
    ...
    ("commented_out_zfill",    21, 's = "{:08b}".format(n)  # was: n.zfill(8)\n'),
    # The str.format method (the correct fix) must NOT flag as the format() builtin.
    ("str_format_method",      21, 's = "{:011b}".format(x)\n'),
    # random.random() is present on MicroPython — only shuffle is absent.
    ("random_random_ok",       21, "j = int(random.random() * n)\n"),
...
# to zero and flips to `fail`: str.zfill, the format() builtin, random.shuffle,
# and the UnicodeDecodeError/UnicodeError exception subclasses. The baseline
# enforces category 21, the tree is clean of it, and a fresh violation would
# block CI.
# ---------------------------------------------------------------------------
