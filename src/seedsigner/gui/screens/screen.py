...
class LogoScreen(BaseScreen):
    ...
    def get_random_partner(self) -> str:
        # Local import: this `random` is for cosmetic UI selection only and is never used
        # for any key/entropy generation. Kept out of module scope to avoid that confusion.
        import random
...
class ScreensaverScreen(LogoScreen):
    ...
    def rand_increment(self):
        # Local import: this `random` only drives the screensaver's bouncing-logo motion
        # and is never used for any key/entropy generation. Kept out of module scope to
